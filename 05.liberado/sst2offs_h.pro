...
pro sst2offs_h,data,Obpos,source,beams,pnt_beam=pntb,pnt_center=pntc,humi=humi,temp=temp,$
...
print,beamcenter,format='("Pointing to = (",f6.2," , ",f6.2,")")'
...
if not keyword_set(temp) then begin
    temp = 0.0
    print,'Assuming Atmospheric Temperature = ',temp,' C'
endif 

if not keyword_set(pres) then begin
    pres=590.0
    print,'Assuming Atmospheric Pressure = ',pres,' mm Hg'
endif 

if not keyword_set(humi) then begin
    humi=20.0
    print,'Assuming Atmospheric Pressure = ',humi,' %'
endif 
...
for i=0,5 do dra[*,i]   = (ra[*,i]  - ephra)  * 3600.
...
source={ew:0.0,ns:0.0,lon:0.0,lat:0.0}
source=replicate(source,ni)
for i = 0l, ni-1 do begin 
    radec_hel,sdra[i]/60.,sddec[i]/60.,pangle,bangle,SRad/60.0,hslat,hslon
    xy=lonlat2xy([hslon,hslat],b0=bangle*!dtor, radius=Srad) 
    source[i].ew=xy[0]
    source[i].ns=xy[1]
    source[i].lon=hslon[i]
    source[i].lat=hslat[i]
endfor

beams={ew:fltarr(6),ns:fltarr(6),lon:fltarr(6),lat:fltarr(6)}
beams=replicate(beams,ni)
for i=0l,ni-1 do begin
  for j=0,5 do begin
     radec_hel,dra[i,j]/60.,ddec[i,j]/60.,pangle,bangle,SRad/60.0,hslat,hslon
     xy=lonlat2xy([hslon,hslat],b0=bangle*!dtor, radius=Srad) 
     beams[i].ew[j]=xy[0]
     beams[i].ns[j]=xy[1]
     beams[i].lon[j]=hslon
